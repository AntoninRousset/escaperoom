...
class EtcdLevelRangeKeys(EtcdLevelRangeIterator):
    ...
    async def __aiter__(self):
        ...
        for key in await self.cluster.range_prefix(self.lr.key,
                                                   keys_only=True):
            if key.is_in_range(self.lr.key, self.lr.start, self.lr.end):
                yield key
...
class EtcdLevelRangeValues(EtcdLevelRangeIterator):
    ...
    async def __aiter__(self):
        for key, value, _ in await self.cluster.range_prefix(self.lr.key):
            if key.is_in_range(self.lr.key, self.lr.start, self.lr.end):
                yield value
