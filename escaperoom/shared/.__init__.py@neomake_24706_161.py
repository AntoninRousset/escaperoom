...
class EtcdCluster(EtcdNode):
    ...
    async def get_value(self, nodepath=None):
        ...
        return await self._get_node(nodepath)['']

    async def get_subnodes(self, nodepath):
        nodepath = nodepath or self.subdir
        return list(filter(None, await self.get_node(nodepath).keys()))

    

    async def _get_node(self, nodepath):
        ...
        async with self.db as f:
            ...
            data = defaultdict(defaultdict, dump(f))

            for k in list(filter(None, str(nodepath).split('/'))):
                data = data[k]
