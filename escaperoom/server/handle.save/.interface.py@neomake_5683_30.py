    ...
    def load_modules(cls, dirpath, factory, id_prefix=''):
        ...
        for path in cls.iter_valid_dir(dirpath):
            ...
            try:
                ...
                LOGGER.exception('Failed to load module %(path)s', path)
...
class TabGroup(Module):
    ...
    def __init__(self, rootdir, dirpath):
        ...
        tabs = self.load_modules(self.path, lambda p: Tab(self.rootdir, p),
                                 f'{self.id}/')
        self.tabs = [tab for tab in sorted(tabs, lambda t: t.id)]
