...
async def perform_modifications(etcd, key, exclude=None, n=64,
    ...
    values = [testutils.random.random_string() for _ in range(n)]

    for _ in range(n // block_size):

        coro = [(etcd.root / expand_key(key)).set() for _ in range(block_size)]
...
async def test_events_node(n=256):
    ...
    async with Etcd() as etcd:
        ...
        node = etcd.root / testutils.random.random_key()
        key = str(node.key)

        await gather(
            perform_modifications(etcd, 'test/', exclude=key, n=n),
