...
async def test_events_node(n=64):
    ...
    async with Etcd() as etcd:
        ...
        await perform_event_test(etcd, node, [
            perform_modifications(etcd, node.key, n=n),
        ], [
            perform_modifications(etcd, f'test/*', exclude={node_name}, n=n),
            perform_modifications(etcd, f'test/{node_name}/*', n=n),
            perform_modifications(etcd, f'test/*/*', n=n),
        ])


async def test_events_subtree(n=64):
    ...
    async with Etcd() as etcd:
        ...
        await perform_event_test(etcd, node.subtree(), [
            perform_modifications(etcd, node.key, n=n),
            perform_modifications(etcd, f'test/{node_name}/*', n=n),
            perform_modifications(etcd, f'test/{node_name}/*/*', n=n),
        ], [
...
async def test_events_children(n=64):
    ...
    async with Etcd() as etcd:
        ...
        await perform_event_test(etcd, node.children(), [
            perform_modifications(etcd, f'test/{node_name}/*', n=n),
        ], [
            perform_modifications(etcd, node.key, n=n),
            perform_modifications(etcd, f'test/{node_name}/*/*', n=n),
...
if __name__ == '__main__':
    ...
    run(test_events_subtree(), 'event on a node subtree')
    run(test_events_children(), 'event on a node children')
